...
def extract_command(message: str):
    """Extrae 'cmd' o 'ret' del JSON si existe"""
    try:
        data = json.loads(message)
        return data.get("cmd") or data.get("ret") or "unknown"
    except Exception:
        return "invalid_json"
...
async def resend_pending_messages(device_id, ws):
    ...
    cmds = [m["cmd"] for m in pending]
    cmds_str = ", ".join(cmds)
    print(f"🔁 Reintentando {len(pending)} mensajes pendientes para {device_id} → {{{cmds_str}}}")

    for m in pending:
        try:
            await ws.send(m["message"])
            m["status"] = "resent"
            print(f"✅ Reenviado mensaje pendiente ({m['cmd']}) → {m['direction']}")
            await asyncio.sleep(MESSAGE_DELAY)
        except Exception as e:
            print(f"⚠️ Error reenviando mensaje pendiente ({m['cmd']}): {e}")
...
async def main():
    print(f"🚀 Servidor puente escuchando en ws://0.0.0.0:7789 (delay={MESSAGE_DELAY}s, caché activa)")
